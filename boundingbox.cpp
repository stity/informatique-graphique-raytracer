...
bool BoundingBox::intersect(const Ray &r) const {
    double tx1 = (bmin[0]-r.C[0])*r.uInv[0];
    double tx2 = (bmax[0]-r.C[0])*r.uInv[0];
    double ty1 = (bmin[1]-r.C[1])*r.uInv[1];
    double ty2 = (bmax[1]-r.C[1])*r.uInv[1];
    double tz1 = (bmin[2]-r.C[2])*r.uInv[2];
    double tz2 = (bmax[2]-r.C[2])*r.uInv[2];
